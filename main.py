...
def main():
    ...
    meta_model = MnistModel().to(device)


    meta_optimizer = FastMetaOptimizer(MetaModel(meta_model), args.num_layers, args.hidden_size).to(device)
    # meta_optimizer = MetaOptimizer(MetaModel(meta_model), args.num_layers, args.hidden_size).to(device)
        ...
        for _ in range(args.updates_per_epoch):
            ...
            for k in range(args.optimizer_steps // args.truncated_bptt_step):
                ...
                for _ in range(args.truncated_bptt_step):
                    ...
                    loss = F.nll_loss(f_x, y)       # calculate loss (negative loglikelihood)
